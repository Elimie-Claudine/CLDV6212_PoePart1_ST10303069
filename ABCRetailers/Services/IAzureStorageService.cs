...
namespace ABCRetailers.Services
{
    public interface IAzureStorageService
    {
        ...
        Task<T?> DeleteEntityAsync<T>(string partitionKey, string rowKey) where T : class, Azure.Data.Tables.ITableEntity, new();


        Task<string> UploadImageAsync(IFormFile file, string containerName);
        Task<string> UploadFileAsync(IFormFile file, string containerName);
        Task<string> DeleteBlobAsync(string blobName, string containerName);
        ...
        Task<byte[]> DownloadFromFileShareAsync(string fileName, string shareName, string directoryName = "");
    }
}
