    ...
    public class AzureStorageService : IAzureStorageService
    {
        ...
        public async Task InitializeStorageAsync()
        {
            try
            {
                ...
                await paymentProofContainer.CreateIfNotExistsAsync(PublicAccessType.Blob);
        ...
        public async Task<T?> DeleteEntityAsync<T>(string partitionKey, string rowKey) where T : class, ITableEntity, new()
        {
            var tableName = GetTableName<T>();
            var tableClient = _tableServiceClient.GetTableClient(tableName);
            ...
            try
            {
                var entity = await GetEntityAsync<T>(partitionKey, rowKey);
                if (entity != null)
                {
        ...
        public async Task<string> DeleteBlobAsync(string blobName, string containerName)
        {
            var containerClient = _blobServiceClient.GetBlobContainerClient(containerName);
        ...
        public async Task<byte[]> DownloadFromFileShareAsync(string fileName, string shareName, string directoryName = "")
        {
            var shareClient = _shareServiceClient.GetShareClient(shareName);
            var directoryClient = string.IsNullOrEmpty(directoryName)
                ? shareClient.GetRootDirectoryClient()
